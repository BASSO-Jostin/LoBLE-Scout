...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0xc000 + 0x0), LENGTH = (976 * 1024 - 0x0 - 0x0)
    RAM (wx) : ORIGIN = 0x10010000, LENGTH = (320 * 1K)
    ITCM : ORIGIN = 268435456, LENGTH = 65536
 ...
 *(".kernel_noinit.*")
    ...
    __kernel_ram_end = 0x10010000 + (320 * 1K);
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    ITCM (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
